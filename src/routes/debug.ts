...
export default async function debugRoutes(fastify: FastifyInstance) {
    ...
    fastify.post('/debug/telemetry', {
        ...
        const { topic, payload } = request.body as { topic: string; payload: any };

        try {
            const buffer = Buffer.from(JSON.stringify(payload));
            telemetryService.ingest(topic, buffer);
            logService.info('system', 'telemetry', `Injected mock telemetry on ${topic}`);
